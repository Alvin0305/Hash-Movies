...
exports.createPlatform = async (req, res) => {
  try {
    ...
    if (req.file) {
      platformData.logo = `/uploads/${req.file.filename}`;
    } else {
      platformData.logo = `/uploads/default-platform.jpeg`;
    }
...
exports.updatePlatform = async (req, res) => {
  try {
    ...
    const platform = await Platform.findByIdAndUpdate(req.params.id, updates, {
      new: true,
      runValidators: true,
    }).select("-__v");
